...
- The user requested ML assignment now and approved simulated records. Lesson 3 now ranks compatible free room/doctor pairs by decision-tree duration estimates from 400 simulated appointments (320 training, 80 chronological test). Inputs are ward/room/doctor categories only. The app displays candidate estimates and simulated evaluation; unknown pairs use labelled training medians, missing model uses labelled first-available assignment without an estimate. Source and regeneration are documented in `data/README.md`. The local `.venv` contains the pinned scikit-learn dependency.
- No real hospital data, personalized duration estimates, ESP32 connection, or external AI integration has been verified. Lesson 3 ML practice remains pending.
