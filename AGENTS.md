...
- Capacity is configurable and saved as of 2026-09-08. `make_demo_resources` takes a `{ward: (rooms, doctors)}` mapping and a set of out-of-service resource IDs; wards absent from the mapping use `DEFAULT_CAPACITY`, so a ward added to `WARD_CODES` later starts with resources rather than none. `MAX_PER_WARD` bounds what the interface will configure.
- Resource IDs are derived from position (`R01-3` is the third room), so growing a ward never renames an existing resource and an existing reservation stays valid. Shrinking removes from the END of the numbering.
- **Reducing capacity must never delete a reserved resource.** `blocking_reservations` reports which reserved IDs a reduction would remove, and the interface refuses the change and names them. Silently dropping a reservation would leave a patient unassigned with nobody told, which the scheduling rules forbid. Taking a reserved resource out of service is refused for the same reason.
- Out-of-service resources are stored separately from capacity counts, so a room under maintenance keeps its ID instead of having it reused by the next room added.
- Three assignment modes exist as of 2026-09-08, all sharing `check_case_is_assignable` and `available_resources` so they cannot disagree about eligibility: `assign_patient` (first fit), `assign_patient_to` (a chosen pair), and `auto_assign` (every waiting case for today, in arrival order).
- **Manual assignment overrides which pair is used, never whether the pair is legal.** The chooser is populated from `available_resources`, so it cannot offer something the rules would refuse, and the rule is still applied on confirm in case the list went stale. Refusals name the specific reason: unknown, wrong ward, out of service, or the ticket already holding it.
- Cases can be marked done as of 2026-09-08, which is the release step the project lacked from lesson 3. `release_reservation` frees the room and doctor, and `storage.set_case_status` records `status='completed'` with a `completed_at` timestamp. A completed case cannot be assigned again and is skipped by automatic assignment.
- Completion is an ARCHIVE, not a deletion: the record stays in the database and is hidden from the working queue behind a Show completed checkbox. There is no reopen, because the released room may already have been given to somebody else.
- Reservations are still not persisted. They mean "right now", so one restored the next morning would hold a room for a visit that already happened. The lifecycle STATUS is persisted, which is the durable fact.
- `storage.migrate` adds columns to a database created before they existed. `CREATE TABLE IF NOT EXISTS` does nothing to an existing table, so any future column must be added there too rather than by telling the user to delete `clinic.db`.
- Automatic assignment is FIRST COME, FIRST SERVED, taking `patient_records` in order. That list is kept in arrival order and reloaded from the database in the same order. The random part of a ticket is not a position and must never be sorted on. A case that cannot be served is left waiting with its reason and does not stop the run, so one full ward never blocks another. Existing reservations, including manual ones, are never reshuffled.
